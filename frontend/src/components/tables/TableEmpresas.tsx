...
import { EmpresaCadastrarModal } from "../ui/modal/Empresa/EmpresaCadastrarModal";
import { EmpresaEditarModal } from "../ui/modal/Empresa/EmpresaEditarModal";
import { EmpresaDeletarModal } from "../ui/modal/Empresa/EmpresaDeletarModal";
import API_BASE from "../../config/api";
import { toast } from "react-toastify";
import { ThemedToastContainer } from "../toast/Toast";
import { EmpresaVisualizarModal } from "../ui/modal/Empresa/EmpresaVisualizarModal";
...
export default function TableEmpresas() {
  ...
  const fetchEmpresas = async (opts?: { initial?: boolean }) => {
    try {
      ...
      if (!res.ok)
        throw new Error(
          `Erro ${res.status}: não foi possível obter os empresas`
        );
  ...
  useEffect(() => {
    fetchEmpresas({ initial: true });

    const start = () => {
      if (pollingRef.current) clearInterval(pollingRef.current);
      pollingRef.current = window.setInterval(() => {
        if (document.visibilityState === "visible") fetchEmpresas();
      }, POLL_MS);
    };
  ...
  const handleCreateEmpresa = async (novoEmpresa: { nome: string }) => {
    try {
      const res = await fetch(`${API_BASE}/empresas`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(novoEmpresa),
      });
      if (!res.ok)
        throw new Error(
          `Erro ${res.status}: não foi possível cadastrar o empresa`
        );
  ...
  const handleUpdateEmpresa = async (payload: {
    id_empresa: number;
    nome: string;
  }) => {
    try {
      ...
      if (!res.ok) {
        const msg = `Erro ${res.status}: não foi possível atualizar o empresa`;
        setError(msg);
        throw new Error(msg);
      }
      etagRef.current = null;
  ...
  const handleDeleteEmpresa = async (id_empresa: number) => {
    try {
      setDeleting(true);

      const res = await fetch(`${API_BASE}/empresas?id_empresa=${id_empresa}`, {
        method: "DELETE",
        cache: "no-cache",
      });

      if (!res.ok) {
        const msg = `Erro ${res.status}: não foi possível excluir o empresa`;
        setError(msg);
        throw new Error(msg);
      }
      ...
      setEmpresas((prev) => prev.filter((u) => u.id_empresa !== id_empresa));
  ...
  const filteredData = useMemo(() => {
    ...
    return empresas.filter((u) => u.nome.toLowerCase().includes(q));
  }, [search, empresas]);
  ...
  return (
    <>
      ...
      <EmpresaVisualizarModal
        isOpen={!!visualizarTarget}
        onClose={() => setVisualizarTarget(null)}
        empresa={visualizarTarget ?? undefined}
        onSubmit={() => {}}
      />
      <EmpresaCadastrarModal
        isOpen={isModalOpen}
      ...
      <EmpresaEditarModal
        ...
        empresa={editTarget ?? undefined}
        onSubmit={handleUpdateEmpresa}
      />
      <EmpresaDeletarModal
        isOpen={!!deleteTarget}
        onClose={() => setDeleteTarget(null)}
        empresa={deleteTarget ?? undefined}
