...
import { SetorCadastrarModal } from "../ui/modal/Setor/SetorCadastrarModal";
import { SetorEditarModal } from "../ui/modal/Setor/SetorEditarModal";
import { SetorDeletarModal } from "../ui/modal/Setor/SetorDeletarModal";
import API_BASE from "../../config/api";
import { toast } from "react-toastify";
import { ThemedToastContainer } from "../toast/Toast";
import { SetorVisualizarModal } from "../ui/modal/Setor/SetorVisualizarModal";
...
export default function TableSetores() {
  ...
  const fetchSetores = async (opts?: { initial?: boolean }) => {
    try {
      ...
      if (!res.ok)
        throw new Error(
          `Erro ${res.status}: não foi possível obter os setores`
        );
  ...
  useEffect(() => {
    fetchSetores({ initial: true });

    const start = () => {
      if (pollingRef.current) clearInterval(pollingRef.current);
      pollingRef.current = window.setInterval(() => {
        if (document.visibilityState === "visible") fetchSetores();
      }, POLL_MS);
    };
  ...
  const handleCreateSetor = async (novoSetor: { nome: string }) => {
    try {
      const res = await fetch(`${API_BASE}/setores`, {
        method: "POST",
      ...
      if (!res.ok)
        throw new Error(
          `Erro ${res.status}: não foi possível cadastrar o setor`
        );
      ...
      setError(err instanceof Error ? err.message : "Erro ao cadastrar setor");
    }
  };

  const handleUpdateSetor = async (payload: {
    id_setor: number;
    nome: string;
  }) => {
    try {
      ...
      if (!res.ok) {
        const msg = `Erro ${res.status}: não foi possível atualizar o setor`;
        setError(msg);
        throw new Error(msg);
      }
      etagRef.current = null;
  ...
  const handleDeleteSetor = async (id_setor: number) => {
    try {
      setDeleting(true);

      const res = await fetch(`${API_BASE}/setores?id_setor=${id_setor}`, {
        method: "DELETE",
        cache: "no-cache",
      });

      if (!res.ok) {
        const msg = `Erro ${res.status}: não foi possível excluir o setor`;
        setError(msg);
        throw new Error(msg);
      }
      ...
      setSetores((prev) => prev.filter((u) => u.id_setor !== id_setor));
  ...
  const filteredData = useMemo(() => {
    ...
    return setores.filter((u) => u.nome.toLowerCase().includes(q));
  }, [search, setores]);
  ...
  return (
    <>
      ...
      <SetorEditarModal
        ...
        setor={editTarget ?? undefined}
        onSubmit={handleUpdateSetor}
      />
      <SetorDeletarModal
        isOpen={!!deleteTarget}
        onClose={() => setDeleteTarget(null)}
        setor={deleteTarget ?? undefined}
